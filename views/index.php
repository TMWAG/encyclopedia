<?php
$page_title = "Онлайн энциклопедия Deep Rock Galactic";
require_once getcwd()."/php/templates/header.php";
?>

<article class="container col-12 rounded bg-primary gy-5 p-2 bg-opacity-75 border-secondary border-2 my-5">
    <div class="row g-2">
        <div class="col-lg-9 col-sm-12 ">
            <div class="row align-items-center justify-content-center g-2">
                <div class="col-12 ">
                    <div class="card bg-primary bg-opacity-75 border-secondary border-2 mb-2">
                        <div class="card-body text-white">
                            <h1 class="text-center fs-2">Добро пожаловать на онлайн энциклопедию Deep Rock Galactic</h1>
                            <hr>
                            <p class="text-center fs-4 text-wrap mb-0">
                                Мы рады приветствовать вас на энциклопедии, посвящённой игре: «Deep Rock Galactic». 
                                Мы стремимся написать о игре как можно больше информации. Хотите помочь? Чего же вы ждёте! 
                                Присоединяйтесь!
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row g-2">
                <div class="col-12">
                    <div class="card bg-primary bg-opacity-75 border-secondary border-2 mb-2 p-3">
                        <div class="card-body text-white p-0">
                            <h2 class="text-center fs-3">
                                Основы
                            </h2>
                            <hr>
                            <div class="row align-items-center justify-content-center g-2">
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=7" class="p-0 d-flex align-items-end justify-content-center flex-wrap ">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Icon_Map_Spacerig.webp" alt="Космическая станция">
                                                <span class="card-text text-wrap text-center link-info">Космическая Станция </span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=8" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Skin_bulldog_default.webp" alt="Экипировка">
                                                <span class="card-text text-center link-info">Экипировка</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=9" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Mining_expedition_icon.webp" alt="Миссии">
                                                <span class="card-text text-center link-info">Миссии</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2 ">
                                        <div class="card-body">
                                            <a href="/article?id=10" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Hoxxes.webp" alt="Хоксесс">
                                                <span class="card-text text-center link-info">Хоксесс</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=11" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Glyphid_swarmer.webp" alt="Существа">
                                                <span class="card-text text-center link-info">Существа</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=12" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Morkite_icon.webp" alt="Ресурсы">
                                                <span class="card-text text-center link-info">Ресурсы</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=13" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Frontpage_perks.webp" alt="Перки">
                                                <span class="card-text text-center link-info">Перки</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2 col-6">
                                    <div class="card bg-primary bg-opacity-10 mb-2">
                                        <div class="card-body">
                                            <a href="/article?id=14" class="p-0 d-flex align-items-end justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/basics/Icon_Accessory.webp" alt="Аксессуары">
                                                <span class="card-text text-center link-info">Аксессуары</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <hr>
                            <div class="row align-items-center justify-content-center g-2">
                                <div class="col-12 col-md-4 text-center">
                                    <a href="/article?id=15" class="link-info">Как играть</a>
                                </div>
                                <div class="col-12 col-md-4 text-center">
                                    <a href="/article?id=16" class="link-info">Требования</a>
                                </div>
                                <div class="col-12 col-md-4 text-center">
                                    <a href="/article?id=17" class="link-info">История обновлений</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row g-2">  
                <div class="col-12 col-md-6 align-self-center">
                    <div class="card bg-primary bg-opacity-75 border-secondary border-2 p-3">
                        <div class="card-body text-white p-0">
                            <h2 class="text-center fs-3">
                                Гномы
                            </h2>
                            <hr>
                            <div class="row align-items-center justify-content-center g-2">
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=6" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/dwarves/Driller_icon.webp" alt="Бурильщик">
                                                <span class="card-text text-center link-info">Бурильщик</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=18" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/dwarves/Engineer_icon.webp" alt="Инженер">
                                                <span class="card-text text-center link-info">Инженер</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=19" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/dwarves/Gunner_icon.webp" alt="Стрелок">
                                                <span class="card-text text-center link-info">Стрелок</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=20" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/dwarves/Scout_icon.webp" alt="Разведчик">
                                                <span class="card-text text-center link-info">Разведчик</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-6">
                    <div class="card bg-primary bg-opacity-75 border-secondary border-2 mb-2 p-3">
                        <div class="card-body text-white p-0">
                            <h2 class="text-center fs-3">
                                Продвинутое
                            </h2>
                            <hr>
                            <div class="row align-items-center justify-content-center g-2">
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=21" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/advanced/Assignment_Icon.webp" alt="Поручения">
                                                <span class="card-text text-center link-info">Поручения</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=22" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/advanced/Icon_Promotion_StarGold.webp" alt="Повышения">
                                                <span class="card-text text-center link-info">Повышения</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=23" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/advanced/DeepDive_Icon.webp" alt="Глубокие Погружения">
                                                <span class="card-text text-center link-info">Глубокие<br>Погружения</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="card bg-primary bg-opacity-10">
                                        <div class="card-body">
                                            <a href="/article?id=24" class="d-flex align-items-end align-self-center justify-content-center flex-wrap">
                                                <img class="card-img" src="/img/pics/mainPage/advanced/Frame_Overclock_Balanced.webp" alt="Разгонные Модули">
                                                <span class="card-text text-center link-info">Разгонные<br>Модули</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row g-2 ">
                <div class="col-12 ">
                    <div class="card bg-primary bg-opacity-75 border-secondary border-2 mb-2 p-3 ">
                        <div class="card-body text-white text-center p-3">
                            <h2 class="text-center fs-3">
                                Принять участие
                            </h2>
                            <hr>
                            <p class="fs-5 text-wrap"> 
                            Хотите помочь лучшить нашу онлайн энциклопедию? Вы можете предложить новую статью или исправить опечатки и грамматические ошибки в уже существующих статьях!</p>
                            <span class="fs-5 text-start">Чтобы получить доступ к полному функционалу вам потребется всего:</span>
                            <ul class="fs-5 text-start">
                                <li>Создать аккаунт</li>
                                <li>Ознакомиться с правилами написания и редактирования статей</li>
                            </ul>
                            
                            
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-3 col-sm-12">
            <div class="row">
                <div class="col-12">
                    <div class="card bg-primary bg-opacity-75 border-2 border-secondary text-white mb-2">
                        <div class="card-body">
                            <h2 class="text-center fs-3">
                                Обновления
                            </h2>
                            <hr>
                            <img class="card-img my-1 rounded border-secondary d-block" src="./img/pics/mainPage/Season_02_banner.webp" alt="Вторжжение конкурентов" title="Сезон 01. Вторжение Конкурентов">
                            <ul class="list-group">
                                <li class="list-group-item ">Версия: 1.36.70593.0</li>
                                <li class="list-group-item ">Дата выхода: 28 Апреля 2022</li>
                            </ul>
                            <div class="card-header">
                                Ключевые особенности:
                            </div>
                            <ul class="list-group list-group-flush bg-opacity-10">
                                <li class="list-group-item">4 Новых Вторичных Оружия</li>
                                <li class="list-group-item">1 Новый Тип События</li>
                                <li class="list-group-item">Боевой Пропуск</li>
                                <li class="list-group-item">Фазионит</li>
                            </ul>
                        </div>
                        <div class="card-footer text-center">
                            <a href="/article?id=17" class="link-info">Читать Статью с Подробностями</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-12">
                    <div class="card text-center bg-primary bg-opacity-75 border-2 border-secondary text-white bg-primary mb-2">
                        <div class="card-body">
                            <h2 class="text-center fs-3">
                                Новости
                            </h2>
                            <hr>
                            <ul class="list-group list-group-flush bg-opacity-10">
                                <li class="list-group-item">
                                    <a href="https://store.steampowered.com/news/app/548430/view/3187996278251338919" target="_blank" class="link-info">
                                        Сезон 02: Патч 02
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://store.steampowered.com/news/app/548430/view/3187995626350226334" target="_blank" class="link-info">
                                        Сезон 02.01: Хотфикс 01
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://store.steampowered.com/news/app/548430/view/3187995626346859329" target="_blank" class="link-info">
                                        Сезон 02: Патч 01
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://store.steampowered.com/news/app/548430/view/3187995626335858519" target="_blank" class="link-info">
                                        Открытие тематического креативного пространства Good Smile для предложений
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-12">
                    <div class="card text-center bg-primary bg-opacity-75 border-2 border-secondary text-white mb-2">
                        <div class="card-body">
                            <h2 class="text-center fs-3">
                                Ссылки
                            </h2>
                            <hr>
                            <ul class="list-group list-group-flush bg-opacity-10">
                                <li class="list-group-item">
                                    <a href="https://store.steampowered.com/app/548430/Deep_Rock_Galactic/" target="_blank" class="link-info">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-steam" viewBox="0 0 16 16">
                                        <path d="M.329 10.333A8.01 8.01 0 0 0 7.99 16C12.414 16 16 12.418 16 8s-3.586-8-8.009-8A8.006 8.006 0 0 0 0 7.468l.003.006 4.304 1.769A2.198 2.198 0 0 1 5.62 8.88l1.96-2.844-.001-.04a3.046 3.046 0 0 1 3.042-3.043 3.046 3.046 0 0 1 3.042 3.043 3.047 3.047 0 0 1-3.111 3.044l-2.804 2a2.223 2.223 0 0 1-3.075 2.11 2.217 2.217 0 0 1-1.312-1.568L.33 10.333Z"/>
                                        <path d="M4.868 12.683a1.715 1.715 0 0 0 1.318-3.165 1.705 1.705 0 0 0-1.263-.02l1.023.424a1.261 1.261 0 1 1-.97 2.33l-.99-.41a1.7 1.7 0 0 0 .882.84Zm3.726-6.687a2.03 2.03 0 0 0 2.027 2.029 2.03 2.03 0 0 0 2.027-2.029 2.03 2.03 0 0 0-2.027-2.027 2.03 2.03 0 0 0-2.027 2.027Zm2.03-1.527a1.524 1.524 0 1 1-.002 3.048 1.524 1.524 0 0 1 .002-3.048Z"/>
                                        </svg>
                                        Steam
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://discordapp.com/invite/DRG" target="_blank" class="link-info">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-discord" viewBox="0 0 16 16">
                                        <path d="M13.545 2.907a13.227 13.227 0 0 0-3.257-1.011.05.05 0 0 0-.052.025c-.141.25-.297.577-.406.833a12.19 12.19 0 0 0-3.658 0 8.258 8.258 0 0 0-.412-.833.051.051 0 0 0-.052-.025c-1.125.194-2.22.534-3.257 1.011a.041.041 0 0 0-.021.018C.356 6.024-.213 9.047.066 12.032c.001.014.01.028.021.037a13.276 13.276 0 0 0 3.995 2.02.05.05 0 0 0 .056-.019c.308-.42.582-.863.818-1.329a.05.05 0 0 0-.01-.059.051.051 0 0 0-.018-.011 8.875 8.875 0 0 1-1.248-.595.05.05 0 0 1-.02-.066.051.051 0 0 1 .015-.019c.084-.063.168-.129.248-.195a.05.05 0 0 1 .051-.007c2.619 1.196 5.454 1.196 8.041 0a.052.052 0 0 1 .053.007c.08.066.164.132.248.195a.051.051 0 0 1-.004.085 8.254 8.254 0 0 1-1.249.594.05.05 0 0 0-.03.03.052.052 0 0 0 .003.041c.24.465.515.909.817 1.329a.05.05 0 0 0 .056.019 13.235 13.235 0 0 0 4.001-2.02.049.049 0 0 0 .021-.037c.334-3.451-.559-6.449-2.366-9.106a.034.034 0 0 0-.02-.019Zm-8.198 7.307c-.789 0-1.438-.724-1.438-1.612 0-.889.637-1.613 1.438-1.613.807 0 1.45.73 1.438 1.613 0 .888-.637 1.612-1.438 1.612Zm5.316 0c-.788 0-1.438-.724-1.438-1.612 0-.889.637-1.613 1.438-1.613.807 0 1.451.73 1.438 1.613 0 .888-.631 1.612-1.438 1.612Z"/>
                                        </svg>
                                        Discord
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://www.microsoft.com/en-us/store/p/deep-rock-galactic-spilpr%C3%B8ve/9nhfvwx1v7qj?ocid=badge&rtc=1" target="_blank" class="link-info">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-xbox" viewBox="0 0 16 16">
                                        <path d="M7.202 15.967a7.987 7.987 0 0 1-3.552-1.26c-.898-.585-1.101-.826-1.101-1.306 0-.965 1.062-2.656 2.879-4.583C6.459 7.723 7.897 6.44 8.052 6.475c.302.068 2.718 2.423 3.622 3.531 1.43 1.753 2.088 3.189 1.754 3.829-.254.486-1.83 1.437-2.987 1.802-.954.301-2.207.429-3.239.33Zm-5.866-3.57C.589 11.253.212 10.127.03 8.497c-.06-.539-.038-.846.137-1.95.218-1.377 1.002-2.97 1.945-3.95.401-.417.437-.427.926-.263.595.2 1.23.638 2.213 1.528l.574.519-.313.385C4.056 6.553 2.52 9.086 1.94 10.653c-.315.852-.442 1.707-.306 2.063.091.24.007.15-.3-.319Zm13.101.195c.074-.36-.019-1.02-.238-1.687-.473-1.443-2.055-4.128-3.508-5.953l-.457-.575.494-.454c.646-.593 1.095-.948 1.58-1.25.381-.237.927-.448 1.161-.448.145 0 .654.528 1.065 1.104a8.372 8.372 0 0 1 1.343 3.102c.153.728.166 2.286.024 3.012a9.495 9.495 0 0 1-.6 1.893c-.179.393-.624 1.156-.82 1.404-.1.128-.1.127-.043-.148ZM7.335 1.952c-.67-.34-1.704-.705-2.276-.803a4.171 4.171 0 0 0-.759-.043c-.471.024-.45 0 .306-.358A7.778 7.778 0 0 1 6.47.128c.8-.169 2.306-.17 3.094-.005.85.18 1.853.552 2.418.9l.168.103-.385-.02c-.766-.038-1.88.27-3.078.853-.361.176-.676.316-.699.312a12.246 12.246 0 0 1-.654-.319Z"/>
                                        </svg>
                                        Microsoft Store
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://www.youtube.com/ghostshipgames" target="_blank" class="link-info">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-youtube" viewBox="0 0 16 16">
                                        <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.007 2.007 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.007 2.007 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31.4 31.4 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.007 2.007 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A99.788 99.788 0 0 1 7.858 2h.193zM6.4 5.209v4.818l4.157-2.408L6.4 5.209z"/>
                                        </svg>
                                        Youtube
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://www.reddit.com/r/DeepRockGalactic/" target="_blank" class="link-info">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-reddit" viewBox="0 0 16 16">
                                        <path d="M6.167 8a.831.831 0 0 0-.83.83c0 .459.372.84.83.831a.831.831 0 0 0 0-1.661zm1.843 3.647c.315 0 1.403-.038 1.976-.611a.232.232 0 0 0 0-.306.213.213 0 0 0-.306 0c-.353.363-1.126.487-1.67.487-.545 0-1.308-.124-1.671-.487a.213.213 0 0 0-.306 0 .213.213 0 0 0 0 .306c.564.563 1.652.61 1.977.61zm.992-2.807c0 .458.373.83.831.83.458 0 .83-.381.83-.83a.831.831 0 0 0-1.66 0z"/>
                                        <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.828-1.165c-.315 0-.602.124-.812.325-.801-.573-1.9-.945-3.121-.993l.534-2.501 1.738.372a.83.83 0 1 0 .83-.869.83.83 0 0 0-.744.468l-1.938-.41a.203.203 0 0 0-.153.028.186.186 0 0 0-.086.134l-.592 2.788c-1.24.038-2.358.41-3.17.992-.21-.2-.496-.324-.81-.324a1.163 1.163 0 0 0-.478 2.224c-.02.115-.029.23-.029.353 0 1.795 2.091 3.256 4.669 3.256 2.577 0 4.668-1.451 4.668-3.256 0-.114-.01-.238-.029-.353.401-.181.688-.592.688-1.069 0-.65-.525-1.165-1.165-1.165z"/>
                                        </svg>
                                        Reddit
                                    </a>
                                </li>
                                <li class="list-group-item">
                                    <a href="https://www.twitch.tv/GhostShip_Games" target="_blank" class="link-info">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-twitch" viewBox="0 0 16 16">
                                        <path d="M3.857 0 1 2.857v10.286h3.429V16l2.857-2.857H9.57L14.714 8V0H3.857zm9.714 7.429-2.285 2.285H9l-2 2v-2H4.429V1.143h9.142v6.286z"/>
                                        <path d="M11.857 3.143h-1.143V6.57h1.143V3.143zm-3.143 0H7.571V6.57h1.143V3.143z"/>
                                        </svg>
                                        GSG Twitch
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</article>
<?php
require getcwd()."/php/templates/footer.php";
?>